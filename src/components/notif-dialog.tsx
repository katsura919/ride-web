      ...
      <DropdownMenuContent align="end" className="w-80">
        ...
        {dummyNotifications.length === 0 ? (
          ...
          dummyNotifications.map((notif) => (
            <DropdownMenuItem key={notif.id} className="flex flex-col items-start py-3 gap-1 cursor-default select-none">
              <span className="font-semibold text-base">{notif.title}</span>
              <span className="text-sm text-gray-700 dark:text-gray-300">{notif.description}</span>
